...
public class Samsung7 extends PowerRunner {
	...
	public static String dailer_Pack = "com.android.contacts";
	...
	// com.android.contacts/com.oneplus.contacts.activities.OPDialtactsActivity

	public static String music_Pack = "com.samsung.android.app.music.chn";
	public static String music_Act = "com.sec.android.app.music.common.activity.MusicMainActivity";

	public static String video_Pack = "com.qiyi.video";
	public static String video_Act = "org.qiyi.android.video.MainActivity";

	public static String camera_Pack = "com.sec.android.app.camera";
	public static String camera_Act = ".Camera";
	...
	public static int call(int times, String number)
		...
		try {
			for (count = 0; count < times; count++) {
				...
				// CM.startActivity(dailer_Pack, dailer_Act);
				CM.hold(2000);
				CM.findObjectBySubText("记录", 2000).click();
				CM.hold(2000);
				CM.findObjectBySubText("10086", 2000).click();
				CM.hold(2000);
				CM.findObjectByRid("com.android.contacts:id/detail_make_call_button", 2000).click();
				CM.hold(30000);
				//挂断
//				if (CM.waitForId("com.android.incallui:id/special_button", 2000))
					ud.click(560,2170);
				...
				if (CM.waitForNewApp(dailer_Pack, 1000)) {
					...
					record(count + " Error");
				CM.waitForId("com.android.contacts:id/detail_make_call_button",
						3000);
				ud.pressBack();
				ud.pressBack();
				CM.hold(5000);
	...
	public static int wxTest(int times) throws Exception {
		...
		try {
			...
			CM.findObjectByRid("com.tencent.mm:id/gx", 1000).click();
			CM.waitForText("发消息", 1000);
			CM.findObjectByText("发消息", 1000).click();

			// com.tencent.mm:id/wf

			if (CM.waitForText("按住 说话", 2000)) {
				CM.findObjectByRid("com.tencent.mm:id/wf", 2000).click();// 切换为键盘模式
				CM.waitForId("com.tencent.mm:id/wh", 1000);
			}

			UiObject editBox = CM.findObjectByRid("com.tencent.mm:id/wh", 1000);
			for (count = 0; count < times; count++) {
				for (int j = 0; j < 3; j++) {
					editBox.click();
					input();
				...
				CM.findObjectByRid("com.tencent.mm:id/wf", 2000);// 切换为 语音模式

				if (CM.waitForText("按住 说话", 2000)) {

					// 发送语音信息
					// CM.findObjectByRid("com.tencent.mm:id/wj", 1000).dragTo(
					// CM.findObjectByRid("com.tencent.mm:id/wj", 1000), 200);//
					// 50
					//CM.findObjectByRid("com.tencent.mm:id/wj", 1000).longClick();
					CM.findObjectByText("按住 说话", 1000).longClick();

					CM.findObjectByRid("com.tencent.mm:id/wf", 2000);// 切换为
																		// 普通消息模式
				}

				CM.waitForId("com.tencent.mm:id/wh", 2000);

				if (CM.waitForId("com.tencent.mm:id/wh", 1000)) {
					if (count % 5 == 0)
						record(count + " OK");
				} else {
					record(count + " Error");
	...
	public static int qqTest(int times) throws Exception {
		...
		try {
			...
			for (count = 0; count < times; count++) {
				...
				CM.findObjectByRid("com.tencent.mobileqq:id/fun_btn", 1000)
						.click();
				if (!CM.waitForId("com.tencent.mobileqq:id/press_to_speak_iv",
						2000)) {
					CM.findObjectByRid("com.tencent.mobileqq:id/fun_btn", 1000)
							.click();
					CM.findObjectByRid("com.tencent.mobileqq:id/fun_btn", 1000)
...
//								600).getBounds();
						ud.drag(1155, 2130, 330, 2120,20);
						if (CM.waitForId(
								"com.android.phone:id/missed_call_buttons", 600))
							CM.findObjectByRid(
	...
	public static int videoTest(int times) throws IOException,
		...
		try {
			...
			if (CM.waitForId("com.qiyi.video:id/phone_cancel_bg", 1000)) {
				CM.findObjectByRid("com.qiyi.video:id/phone_cancel_bg", 1000);
			}

			CM.findObjectByRid("com.qiyi.video:id/naviOff", 1000).click();
			CM.findObjectByText("观看本地视频", 1000).click();
			CM.clickOnText("HeKnowNo.mp4", false, true, true);
	...
	public static int browserTest(int times) throws IOException,
		...
		try {
			startMusic();
			ud.pressHome();
			CM.startActivity("com.sec.android.app.sbrowser",
					".SBrowserMainActivity");
			if (CM.waitForNewApp("com.sec.android.app.sbrowser", 2000)) {
				...
				for (count = 0; count < times; count++) {
					for (int i = 0; i < 4; i++) {
						...
						CM.hold(2000); // 主页图标
						ud.click(550, 560);// 百度图标

						CM.hold(9000); // 主页图标
						ud.click(700, 2400);
						if (CM.waitForNewApp("com.sec.android.app.sbrowser", 1000)) {
							if (count % 5 == 0)
								record(count + " OK");
						} else {
							record(count + " Error");
							ud.pressHome();
							CM.stopPackage("com.sec.android.app.sbrowser");
							break;
						}
						CM.hold(5000);
					}
				...
				CM.hold(2000);
				CM.waitForId("com.android.chrome:id/bookmarks_button", 2000);
				for (count = 0; count < times; count++) {
					for (int i = 0; i < 4; i++) {
						CM.findObjectByRid(
								"com.android.chrome:id/most_visited_thumbnail",
								1000).click();
						CM.hold(9000);
						ud.pressBack();
						if (CM.waitForId(
	...
	public static int cameraTest(int times) throws UiObjectNotFoundException {
		...
		try {
			...
			for (count = 0; count < times; count++) {
				...
				for (int j = 0; j < 3; j++)// takephoto capture
				{
					CM.findObjectByText(
							"快门",
							1000).click();
...
//						5);
//				CM.hold(1000);
//				if (CM.waitForText("录制", 1000))
//					CM.findObjectByText("录制", 1000).click();
//				CM.hold(3000);
				CM.findObjectByText(
						"录制", 1000)
						.click();
...
//					}
	...
	private static void startMusic() throws UiObjectNotFoundException {
		...
		CM.hold(3000);
		CM.findObjectByRid("com.samsung.android.app.music.chn:id/play_pause_icon", 1000)
				.click();
		ud.pressBack();
		ud.pressBack();
	}
