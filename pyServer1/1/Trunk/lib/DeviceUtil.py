...
def unlockScreen(strDeviceID):
    ...
    try:
        ...
        AdbOperation.executeAdbCmd(['input', 'keyevent', 'KEYCODE_HOME'], strDeviceID, True)            # home
...
def initTestDevice(strDeviceID):
    if AdbOperation.isDeviceExists(strDeviceID) is False:
    ...
    configProvision(strDeviceID)

    logger.info("Set Screen Stay On")
    setScreenStayOn(strDeviceID)

    logger.info("Config Wifi")
    configWifi(strDeviceID)

    logger.info("Remove Asserttip")
    removeAsserttip(strDeviceID)

    logger.info("Reboot Device")
    return rebootDevice(strDeviceID)
